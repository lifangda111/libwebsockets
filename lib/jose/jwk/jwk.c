	       ...
	       enum lws_gencrypto_kty kty, int bits, const char *curve)
{
	...
	case LWS_GENCRYPTO_KTY_RSA:
	{
		...
		break;
	case LWS_GENCRYPTO_KTY_OCT:
		n = lwsl_gencrypto_bits_to_bytes(bits);
...
lws_jwk_export(struct lws_jwk *jwk, int private, char *p, size_t len)
{
	...
	case LWS_GENCRYPTO_KTY_RSA:
		...
		limit = LWS_GENCRYPTO_RSA_KEYEL_COUNT;
		if (!private)
		...
		for (n = 0; n < limit; n++) {
			...
			if (n)
				p += lws_snprintf(p, end - p, ",");
			p += lws_snprintf(p, end - p, "\"%s\":\"", jwk_tok[n + 1]);
	...
	case LWS_GENCRYPTO_KTY_EC:
		...
		for (n = 0; n < LWS_GENCRYPTO_EC_KEYEL_COUNT; n++) {
			...
			if (!private && n == LWS_GENCRYPTO_EC_KEYEL_D)
				continue;

			if (n)
				p += lws_snprintf(p, end - p, ",");
			p += lws_snprintf(p, end - p, "\"%s\":\"", ec_names[n]);

			if (n == LWS_GENCRYPTO_EC_KEYEL_CRV) {
				m = jwk->e[n].len - 1;
				memcpy(p, jwk->e[n].buf, m);
			} else {
				m = lws_jws_base64_enc((const char *)jwk->e[n].buf,
						      jwk->e[n].len, p,
						      end - p - 4);
