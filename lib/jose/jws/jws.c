		       ...
		       int out_len)
{
	...
	if (blocks > LWS_JWS_MAX_CONCAT_BLOCKS)
		return -1;
			            ...
			            struct lws_context *context)
{
	...
	n = lws_jws_compact_decode_map(map_b64, &map, buf, sizeof(buf));
	if (n > 3 || n < 0)
		return -1;
			        ...
			        struct lws_context *context)
{
	...
	n = lws_jws_compact_decode(in, len, &map, &map_b64, buf, sizeof(buf));
	if (n > 3 || n < 0)
		return -1;
		      ...
		      char *b64_sig, size_t sig_len)
{
	...
	case LWS_JOSE_ENCTYPE_ECDSA:
		...
		m = lwsl_gencrypto_bits_to_bytes(jose->alg->keybits_fixed) * 2;
